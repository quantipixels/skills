...
At the start of a conversation, use `configure-qp` in inspect mode to resolve communication preferences. Resolve worker model/reasoning preferences before staffing. Missing settings require no setup. Keep user decisions and final synthesis in the main conversation. In a delegated thread, follow the bounded assignment rather than inheriting the main-agent role.
