...
State the durable postcondition and the transaction that should own it. Exercise the public operation against the supported storage engine when its semantics matter. Establish the fixture's required service capability, not just an open port; apply actual migrations to an empty or identified upgrade snapshot according to the question. Substitute engines may differ in locking, constraints and transaction behavior. Commit through the application's real boundary, then read through a fresh transaction/context so an identity map or cached object cannot impersonate persisted data. Verify identity, important values and relationships after hydration. A test's automatic rollback can hide commit-time failures or callbacks; match the claimed behavior.
...
Reuse existing fixtures and failpoints. Fault injection belongs in an authorized isolated test environment, never an inferred permission to disrupt live services. If the needed boundary cannot be exercised, report the exact unproved claim.
