...
Reuse existing fixtures and failpoints. Resolve non-obvious database engine, readiness or command questions from the project and host. Fault injection belongs in an authorized isolated test environment, never an inferred permission to disrupt live services. If the needed boundary cannot be exercised, report the exact unproved claim.
