...
Prefer extending the existing owner when the semantics fit. Add a separate implementation only when no suitable one exists or extension would compromise correctness, ownership, compatibility or maintainability; explain the concrete reason. Similar-looking code alone does not justify a shared abstraction. Choose the smallest idiomatic change that preserves required data and unrelated work. Check language, framework and provider guarantees before adding custom state or coordination. Fix the cause rather than accumulating workarounds; surface consequential scope expansion before taking it on.
