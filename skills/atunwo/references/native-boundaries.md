...
For dynamic native evidence, confirm instrumentation and target reachability. A fuzz harness must bound resources, reset per-input state and catch only expected rejection; harness faults are not product defects. Preserve capture/build identity and the reproducer. When reviewing a supplied fix, replay the reproducer with a valid control if execution is authorized; otherwise report the missing runtime evidence. Review does not authorize implementing the correction. A clean random run does not prove a specific defect was fixed.
