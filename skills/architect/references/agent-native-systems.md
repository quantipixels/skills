...
Test only scenarios that distinguish the design: insufficient context, stale identity/permission, lost receipt and retry, interruption, human-only refusal, or divergent human/agent views. Use the owning architecture result rather than a separate readiness score.
