...
When the candidate changes domain language, context boundaries, ADR state, canonical project knowledge, or `.nongoal`, require the owning workflow's exact-current `alakowe` result. Verify its candidate identity, authority, destinations, and verdict without repeating domain or documentation discovery. Treat a missing, blocked, contradictory, or stale result as an evidence gap.
