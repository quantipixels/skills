...
2. Pin the user/job, primary surfaces, platform constraints, content/data shape, accessibility needs, density, and existing design decisions. When reviewing implementation, also pin the accepted direction and exact rendered candidate/evidence. When unresolved domain concepts materially determine information architecture, labels, workflow states, or conceptual grouping, use `amose` for that clarification instead of inventing product vocabulary inside UX design. Reading established product/domain language does not require an `amose` handoff.
