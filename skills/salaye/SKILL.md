...
Help the user understand and examine one current focus through an explicit conversational protocol. Stay responsive to the user's questions and steering. Own the exploration, not the agent session, living-artifact lifecycle, or every specialist result.
...
- For orientation, progressive exploration, or explanation, read [explore-explain.md](references/explore-explain.md).
- For a focused question, analysis, investigation, or research, read [investigate.md](references/investigate.md).
- For assessment against criteria or a conversational review, read [evaluate.md](references/evaluate.md).

Modes can run in an agreed sequence. User steering changes the lens or inspection priority without silently discarding material evidence. When decisions, dependencies, consequence branches, or gaps shape the inquiry, read [decision-awareness.md](references/decision-awareness.md).

When the user supplies a skill, instruction, checklist, method, lens, or evidence source, read [protocol-composition.md](references/protocol-composition.md). Follow an available named skill through its own contract. Do not imitate an unavailable skill or let a supplied protocol expand authority silently.

## 3. Work conversationally
...
Obtain an exact-current specialist result when the requested outcome has another owner. Keep its candidate identity, evidence boundary, freshness, authority, limitations, and side effects intact. Do not strengthen, relabel, or silently combine it with a conversational conclusion.

In particular, `arojinle` owns complete material decision closure, `atona` owns a maintained architecture or migration plan, `qp-code-review` owns a code-review verdict, `simplify` owns maintainability review, `iwadi` owns a durable research record, and implementation owners own source changes. Ṣàlàyé can explain, question, or navigate these results but cannot replace them.
