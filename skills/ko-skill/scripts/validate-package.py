...
from package_metadata import UniqueKeyLoader, read_frontmatter, skill_directories
...
def validate_skill(repo: Path, skill_dir: Path) -> list[Finding]:
    ...
    if len(relative_dir.parts) != 2 or relative_dir.parts[:1] != ("skills",):
        findings.append(Finding("skill.group", str(relative_dir), "skill must be directly under skills/"))
