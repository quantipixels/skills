...
When a consequential rollout needs operational acceptance or data recovery, read [rollout readiness](rollout-readiness.md). Keep its checks and stop conditions in the current plan or runbook; delivery and live execution retain their existing owners and authority.

Use [managed initiatives](managed-initiative.md) only when the governing workflow requires named readiness states, coordinated multi-candidate delivery, or a durable lifecycle record. Its formal gates supplement this workflow; size alone does not require them.
...
When delivery has multiple work units or candidates, dependencies, owners, or a multi-session handoff, read [delivery tracking](delivery-tracking.md). Keep execution and proof details with their owners; Atọ́nà owns whether the combined results satisfy the initiative.
...
Keep ordinary rationale in the HTML plan; read [durable reconciliation](durable-reconciliation.md) only when required governing knowledge needs updating.
