...
Order only the necessary changeover steps, with prerequisites and ownership. Include backfill progress, lock/resource bounds, supported old/new coexistence and restart points where they control safety. Reuse `alaga`'s populated-data audit and migration proof rather than specifying another test suite. An empty-database pass does not establish launch readiness.
