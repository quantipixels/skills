...
Keep shared project knowledge in the repository or its established shared destination, following [records](../../productivity/records.md); private task storage is not its home. Link the useful documents from the entrypoint contributors or agents actually read. Reuse the same standards during construction and local review; avoid copies that can diverge.
