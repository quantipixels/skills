...
Historical reconstruction and evidence-backed improvement judgments stay with [agent session](agent-session.md) and [corpus analysis](corpus-analysis.md). Use [oro-sigidi](../../../commands/oro-sigidi.md) to review or revise agent-facing instructions when that follow-on work is authorized.
...
Leave output on stdout for one-session use. When an index is useful for resumption or repeated analysis, save the JSON through [records](../../productivity/records.md) and keep its corpus boundary and source locators with it. This is working evidence, not project knowledge by itself.
...
- `COMMAND_PATH_REFERENCE` — the record contains an Alárinà command-reference path; this is a locator, not proof that the command was read or applied; and
- `OWNER_PATH_REFERENCE` — the record contains a legacy bundled-owner path, useful when examining earlier installations.
