...
Prefer established terms with strong model priors. Apply the engineering-principle guidance in [agent-facing writing](../../commands/oro-sigidi.md); an acronym saves context only when it preserves the intended decision. Coin a new term only when repeated use repays its decoding cost.
...
- Project or host capability guidance for installation, configuration, authentication, integration, or readiness.
