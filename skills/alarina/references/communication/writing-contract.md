...
When creating or materially improving a skill, consider prose, worked examples, counterexamples, semantic templates, ready-made assets and deterministic operations. Read [resource boundaries](instructions/resource-boundaries.md) when that choice changes repeated effort, correctness or maintenance. A model's ability to generate something is not evidence that regeneration is better than reuse.

Keep resources with their consuming skill. Prefer existing project or native capabilities; [alaga-deliver](../../commands/alaga-deliver.md) owns substantial implementation and runtime proof. Authoring alone grants no installation, execution or publication authority.
...
When authoring reusable project knowledge or instructions, check [knowledge discoverability](../productivity/knowledge-discoverability.md) at the affected reader entry point; return any out-of-scope pointer correction to the caller.
