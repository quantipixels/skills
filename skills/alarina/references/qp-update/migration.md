...
- **Codex project:** add `For software-project work, use the installed $qp-skills:alarina skill and load the relevant command; follow the user's narrower request.` to the project's `AGENTS.md`. For a personal preference across projects, use `~/.codex/AGENTS.md`. Codex does not receive a native Alárinà plugin agent in this package; this instruction selects the skill, not an agent profile. See [Codex instruction scopes](https://learn.chatgpt.com/docs/agent-configuration/agents-md).
