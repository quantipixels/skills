...
The single-entry release replaces all former QP specialist and utility entrypoints with `alarina`; their methods now live under `skills/alarina/commands/`. For a native plugin this is one package update. For a selective installation, adopting the whole Alárinà replacement requires existing replacement authority; do not silently expand a one-skill selection. An old updater may look for the retired `skills/qp-update/SKILL.md`: establish the documented relocation to `skills/alarina/commands/qp-update.md` from the same trusted source revision before following the current procedure. A missing old path alone is insufficient evidence.
