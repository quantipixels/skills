...
- **Codex project:** add `For software-project work, use the installed $qp-skills:alarina skill and load the relevant command; follow the user's narrower request.` to the project's `AGENTS.md`. For a personal preference across projects, use `~/.codex/AGENTS.md`. This selects the skill in the current agent. For a named custom agent, the root package supplies `agents/alarina.codex.toml`; copy it as `alarina.toml` into the project's `.codex/agents/` or personal `~/.codex/agents/` only when that setup is requested, preserving existing customizations. Codex does not auto-register it from the plugin. See [Codex custom agents](https://learn.chatgpt.com/docs/agent-configuration/subagents) and [instruction scopes](https://learn.chatgpt.com/docs/agent-configuration/agents-md).
