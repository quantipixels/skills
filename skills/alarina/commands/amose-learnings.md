...
Expensive research whose loss would cause rediscovery, but not a consequential wrong action, does not enter `.learnings`. Use [iwadi](iwadi.md) and an existing research destination when independent persistence is warranted. Consumers can retrieve that research without promoting it into a governing rule.
...
Compact relevant sections when repetition or stale material impairs use. Do not impose an arbitrary size limit. Consumers may read only relevant sections; passive reading does not require invoking [amose](amose.md).

When a relevant knowledge store is too large for a useful full read, search its existing records by canonical domain terms, symptoms, decisions and the conditions under which a lesson applies. Use the record's format and read substantive matches for current applicability before returning only what changes the task. No match is valid when the relevant destination and search coverage were checked; excluded records, parse failures and truncated results leave uncertainty. A missed prior lesson may justify one useful pointer saying where and when to look; it does not justify another index, a mandatory archive scan or broader admission to `.learnings`. Keep independently useful research with [iwadi](iwadi.md) and current work with its existing owner.
