...
| Plain-language explanation | `salaye` |
| Visual explanation of relationships | `fihanmi` |
| Standalone browser report or presentation deck | `html-artifact` or `slides` respectively |
| Technical prose or prose cleanup | `technical-writing` or `yo-slop` respectively |
...
- Work management: `pepeye` for supervised delegation, `scope-guard` for explicit scope steering, `handoff` for session transfer, `akosile` for selected shared workspace storage.
