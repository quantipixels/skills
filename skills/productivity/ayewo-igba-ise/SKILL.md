...
Route recurring evidence to the earliest owner that can prevent it rather than making downstream cleanup better: skipped/reopened consequential plan choices → `atona`/`arojinle`; architecture drift/missing invariant → `solution-architect`; TDD/test accumulation → `alaga`; accidental complexity/state/abstraction bloat → `pare`; redundant proof requests → `atunwo`; a reliably mechanical coding pattern → the repository tool through `irinse`; repository-specific durable pattern → `amose`; selector/skill contract defect → `ko-skill`/`alarina`. Recommend changes to Experimental `akowe-code` only when it was explicitly in the evaluated path or separate durable evidence proves an experiment-wide defect. Runtime findings never auto-mutate shared guidance or project knowledge.
