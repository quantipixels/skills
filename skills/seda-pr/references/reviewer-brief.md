...
Read the actual candidate/diff, applicable domain language, relevant decisions and available proof. Pin head/base for live repository claims; supplied partial evidence stays partial. Use established `amose` results or the project's canonical terms, states, actors and invariants; invoke `amose` only for consequential unresolved meaning. Do not invent a glossary or substitute implementation names for business concepts.
...
For consequential changes, include established rollout guards, stop signals and recovery actions, or state the missing proof. A flag or down migration does not recover deleted data or recall external effects. Assessment authorizes no destructive exercise; an author assessment is not independent approval. Use `atunwo` for material contested judgment, not automatically for every body.
...
Adapted in original wording from [HumanLayer's visual-pr and its description template](https://github.com/humanlayer/skills/tree/ca7c8088db69e315a8b2deea43820270457f8f3c/plugins/visual-pr/skills/visual-pr): purpose first, caveats early and a change-shaped outline. QP retains domain language, evidence, recovery and authority boundaries rather than adopting the upstream template verbatim.
