...
Establish inspection, setup/update, or removal and whether the target is user scope or one repository. Reuse choices already established in the conversation.
...
The optional Pepeye binding is: “Use `pepeye` when delegation or a bounded multi-stage workflow would materially help.” Confirm the skill is installed and discoverable before adding this binding. Setup does not install missing skills without authorization.
...
When the user asks to replace or clean up overlapping guidance and install the shipped instructions, prepare one consolidated diff: copy the selected general instructions from `assets/instructions.md` verbatim into the managed block, include the exact optional Pepeye binding above if selected, and remove equivalent guidance outside the block. Preserve personal instructions and additional rules that the shipped text does not cover, including unique clauses in otherwise overlapping paragraphs. Show any customized binding conditions that the shipped binding would replace. The diff approval covers both installation and the shown removals. Plain inspection or setup without a cleanup request retains the default preservation behavior.
...
Inspect the same scope, preview cleanup, obtain permission, and back up affected files first. Remove only the managed instruction block. Leave surrounding content intact; delete an instruction file only when removal leaves it empty and deletion was approved.
...
Report the selected scope, changed files, applied options, backup paths, and skipped or unsupported choices. A new Codex session may be required to load changed instructions/settings. File validation does not prove skill selection, model availability, or experiment activation.
