...
Match freedom and precision to risk: use judgment for variable work, constrained patterns for preferred paths, and deterministic scripts for fragile repeated operations. Before adding, keeping, expanding, moving, or replacing a bundled script, read [bundled-script boundary](references/script-boundary.md) and record one disposition for that script. A script is not earned by repetition alone; it must own one necessary mechanical `input → deterministic output` seam while the skill retains semantic judgment and authority.
