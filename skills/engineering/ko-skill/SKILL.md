...
Write for a capable agent. State semantic behavior, invariants, authority and completion; let the agent adapt ordinary search, shell/filesystem work, Git use, editing, tool discovery, delegation/subagents, and equivalent orchestration to its available environment. Do not pre-author capability fallback trees the agent can infer. Concrete mechanics earn instruction space when the mechanism establishes correctness, authority, determinism, non-obvious safety, a machine/external protocol, or a compatibility boundary, or when a small operational anchor materially reduces recurring rediscovery of how to enter a capability. An operational anchor can point to authoritative current documentation or a canonical interface and include one representative invocation or discovery command; keep volatile details subject to revalidation instead of expanding the anchor into a command catalogue.
