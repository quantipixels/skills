...
def _resolve_subject(
    repo: Path, owner: str, subject: str, *, slug: str | None, create: bool
) -> dict[str, Any]:
...
def resolve_record(
    ...
    *,
    ...
    if not create:
        return _resolve_subject(repo, owner, subject, slug=slug, create=False)
    initialize_dirs(repo)
    with workspace_lock(repo, f"subject:{owner}:{subject}"):
        return _resolve_subject(repo, owner, subject, slug=slug, create=True)
...
def write_record(
    ...
    required_string(frontmatter, "record_type")
    ...
    with workspace_lock(repo, f"subject:{owner}:{subject}"):
        ...
        other_matches = [
            item
            for item in records
            if item["normalized"]["subject"] == subject
            and item["path"].resolve(strict=False) != path.resolve(strict=False)
        ]
        if other_matches:
            raise WorkspaceError(
                "DUPLICATE_SUBJECT",
                "Another record already owns this owner and subject",
                owner=owner,
                subject=subject,
                records=[workspace_path(repo, item["path"]) for item in other_matches],
            )
        ...
        with workspace_lock(repo, f"record:{record_ref}"):
            ...
            if current and current["digest"] != expected:
                raise WorkspaceError(
                    "STALE_WRITE",
                    "Record changed since the caller's exact read",
            ...
            assigning_subject = current is None or current["normalized"]["subject"] is None
            if assigning_subject and invalid:
                raise WorkspaceError(
                    "INVALID_OWNER_RECORDS",
                    "Cannot assign a subject while sibling owner records are invalid",
                    invalid=invalid,
                )
    ...
    except (WorkspaceError, OSError, Timeout) as error:
        if isinstance(error, WorkspaceError):
            payload = error.payload()
        elif isinstance(error, Timeout):
            payload = {"code": "INDEX_LOCK_TIMEOUT", "message": str(error)}
        else:
            payload = {"code": "INDEX_WRITE_FAILED", "message": str(error)}
        index = {"state": "FAILED", "error": payload}
    return {
        **record_result(repo, owner, record_id, True),
        "revision": revision,
