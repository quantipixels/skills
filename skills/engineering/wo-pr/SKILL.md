...
3. In a stack, choose the **active frontier**: the lowest layer in the stewardship set that still needs work or refreshed evidence and whose lower ancestors have stable current epochs with no unresolved source-changing blocker. A lower ancestor that is already `PROVIDER_READY` is a satisfied prerequisite even while it remains open; merge is not required to advance. Hold higher layers only behind a changing/blocking ancestor as `HELD_BY_ANCESTOR`; do not keep reviewing or repairing them against a base expected to move.
4. Process only the active frontier's current epoch. If no layer still needs work or refreshed evidence, refresh the set once and evaluate readiness instead of selecting an already-ready layer again. Validate obvious reviewer feedback directly from current evidence. Use a separate triage result only when claim validity, scope, duplication, or required information is materially uncertain. Reviewer feedback is evidence, not mutation authority.
