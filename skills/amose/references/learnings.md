...
When a relevant knowledge store is too large for a useful full read, search its existing records by canonical domain terms, symptoms, decisions and the conditions under which a lesson applies. Read the substantive matches and check current applicability before returning only the constraints or evidence that change the task. No match is valid. A missed prior lesson may justify one useful pointer saying where and when to look; it does not justify another index, a mandatory archive scan or broader admission to `.learnings`. Keep independently useful research with `iwadi` and current work with its existing owner.
