...
2. Pin the user/job, primary surfaces, platform constraints, content/data shape, accessibility needs, density, and existing design decisions. When reviewing implementation, also pin the accepted direction and exact rendered candidate/evidence. Use `amose` as needed.
...
8. Use `brand` as needed and `alaga` for requested implementation.
