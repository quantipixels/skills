...
When an anonymous or keyless mode is currently supported, prefer it for bounded public-web retrieval that fits its documented limits. Do not create an account, request a key, store credentials, or switch to a paid path without the required setup authority.
