...
Use when a supplied CPU profile, heap snapshot, stack sample or execution trace needs reduction, or when a reproducible resource fault needs its first bounded capture. Irinse owns selecting and interpreting the capture tool. `alaga` owns the causal diagnosis and any correction; return the reduced evidence there rather than promoting a hotspot or correlation into a verdict.
