...
Discover the supported engine/version, driver/ORM, migration mechanism and build. Reuse existing disposable infrastructure. Testcontainers can provision a real engine when available, but a running process or open port is only basic readiness. Wait for the service capability the tests need, use assigned connection details, apply production migrations and isolate fixture data. Check from an empty database or an identified upgrade snapshot according to the proof question; neither substitutes for the other. Choose connection coordinates from the caller: host tests use the reported hostname and mapped host port; peer containers use a network alias and internal service port. Do not point migrations at an unverified shared or live target.
