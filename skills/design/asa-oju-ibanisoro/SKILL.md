...
2. When an accepted `amoye-ui-ux` direction materially governs the result and visual/interaction fidelity remains consequential or uncertain, return the exact current render/evidence plus accepted direction to Amọ̀ye for review. Do not ask Amọ̀ye to infer the result from source code when a rendered surface can establish it.
