...
For one compatibility release, the compiler also accepts legacy `--config`, `--output`, and `--format css|tailwind` forms; those operational flags emit a deprecation notice. `--help` remains ordinary help. Move callers to positional input, redirected CSS stdout, and project-owned Tailwind mapping before the next breaking release.
