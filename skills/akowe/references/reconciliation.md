...
Check relevant instructions at their point of use: a runbook or local skill can contradict a correct overview without containing any broken links. Correct a proved instruction error through `oro`'s agent-facing branch when edit authority covers it; unresolved meaning returns to `amose` or `architect`. Do not propagate the same unsupported claim into every document to make them agree.
...
A schema describing a real API is not necessarily a documentation-only file. When its correction would alter the product contract or generated clients, return that obligation to `alaga` and the caller. If generation is unavailable, report the source correction and stale output separately; do not call the generated reference current.
