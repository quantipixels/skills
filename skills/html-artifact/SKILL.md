...
Choose one visual premise from the subject and reader's task. Let it shape composition, type, density and the central representation, not only an accent color. Follow supplied direction and project conventions first. A quiet operational view and an expressive explainer deserve equal craft; neither needs a prescribed palette, font pairing, hero or card grid. Reuse familiar controls without inheriting a previous page's entire silhouette. When model-default styling or composition persists, use `visual-taste` for the corrective design judgment.
