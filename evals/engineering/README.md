...
Use this opt-in standard-library kit when the question requires actual coding outcomes. For selecting proportionate proof, start with the [focused checks guide](../README.md). The native host runs models; this kit freezes tasks, checks returned artifacts and summarizes evidence.
...
Use `adanwo` in measured-experiment mode for experimental controls and outcome judgment. Enforce the declared actor budget in the native host with no automatic retries. The checker's `--timeout` bounds each local subprocess, not model execution.
