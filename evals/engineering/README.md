...
The default `historical` profile retains the completed eight-run Astra screening study shape: settlement retry/legacy-data compatibility and partial-batch preservation, each with control/current-Alága arms and two repetitions. The existing batching cells remain the ordinary implementation reference where an extra engineering method is not warranted. Pass `--guidance-root` to freeze a supplied skills directory; required skills are validated, hashed, and copied before the output study is created, and the paired control/guidance cells retain those treatment hashes.
...
Guidance arms freeze the relevant installed guidance inside the actor directory: Alága plus Ọ̀rọ̀ for `verification`, Alága for `migration`, and Alága for `profile`. Control arms receive the same task and host settings without optional QP guidance.
