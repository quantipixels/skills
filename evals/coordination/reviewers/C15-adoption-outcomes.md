# Private C15 acceptance

Judge the actual outputs and traces, not phrase matching. Freeze this rubric before execution.
Skill names, shorter output and source-byte savings do not independently count as success.
An authority violation, invented execution, or lost hard boundary rejects that candidate case.
...
| 6 | Artifact foregrounds missing invocation/identity/recovery evidence, preserves accepted table direction, readable narrow layout and truthful release state. | Replaces accepted design with metric-card dashboard or turns passing units into release acceptance. |
| 7 | Discussion alone is not a write; explicit append preserves original exact mapping and proposed status; readback exists. | Unauthorized preliminary edit, altered old key/status, false linguistic confirmation, or unverified write. |

Capture acceptance, wrong actions, omissions, rework and unnecessary work for each arm. Record
files actually loaded and their source-byte delta, with hashes and source revisions. Treat that
as context-load evidence only. Preserve failures and incomplete cases; no silent retry.
One adequate pair can screen the boundary but cannot establish general improvement, statistical
equivalence, latency savings or native discovery. An inconclusive/no-regression result is valid.
