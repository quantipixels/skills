...
Keep a run report in the active tree only while it supports a current decision or unresolved investigation. When superseded, remove it and any unused replay artifacts; link the immutable Git source when provenance remains useful. Do not rewrite historical verdicts or maintain a second archive directory. Useful cases, executable fixtures and regression checks have their own retention decision; an old run does not make them obsolete.

Generated runs and private traces stay ignored in `.qp/` or external storage. Changing a task, rubric or invocation condition invalidates direct comparison with older runs. Earlier structure drew on [SureForge](vendor/sureforge/UPSTREAM.md); attribution and license remain.

The decision-first method also draws on [eval-audit](https://github.com/ai-evals-course/evals-skills/blob/2edbc5b1b0dc91f74fcfa8fd8f7eaeb302e052ab/skills/eval-audit/SKILL.md) and [validate-evaluator](https://github.com/ai-evals-course/evals-skills/blob/2edbc5b1b0dc91f74fcfa8fd8f7eaeb302e052ab/skills/validate-evaluator/SKILL.md): inspect actual failures and qualify interpretive judges. This repository does not adopt universal sample quotas, accuracy targets or a new review application.
