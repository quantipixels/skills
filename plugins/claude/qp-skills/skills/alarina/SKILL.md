---
name: alarina
description: Carry software-engineering work through investigation, diagnosis, planning,
  specifications, work decomposition, architecture, implementation, testing, review,
  incident recovery, performance improvement, migration, documentation, PRs and releases.
  Use for unclear engineering ownership, multi-stage work or resuming an accepted
  task; also for prototypes, agent instructions, human-facing writing, technical explanations,
  HTML artifacts, domain records, macOS disk cleanup and Yoruba language or terminology
  guidance. Accept named Alarina commands and natural requests. Private serving (pese)
  and installation updates (qp-update) require explicit user invocation of those commands.
  Exclude casual unrelated facts and standalone visual design.
when_to_use: Select Alárinà for engineering investigation, planning, specifications,
  work packages, implementation, tests, architecture, review, incidents, documentation,
  agent instructions, PRs and releases; also requested writing, prototypes, HTML explanations,
  macOS disk cleanup or Yorùbá guidance. Internal pese and qp-update commands require
  explicit user invocation. Do not infer those commands or turn routing advice into
  execution.
...
3. Open conditional shared resources only when their load condition applies. Reuse already-read current content in the same session. Do not load every command or ask for separately installed QP specialists: the complete methods ship in this tree. Read applicable external skill definitions when their expertise materially helps; report a required missing capability rather than pretending to invoke it.
...
When no playbook fits, compose the smallest adequate path from the command table and applicable external capabilities. Project-local instructions or playbooks can refine it within existing authority. A new combination does not require a permanent playbook, and unfamiliarity alone does not require a plan.
...
Keep cohesive work in the main thread by default. Delegate only when parallel execution, separate context or independent scrutiny earns briefing and integration costs. For useful delegation read [coordination](references/coordination.md); when model, effort or provider-native controls matter also read [host policy](references/host-policy.md). Use native collaboration controls rather than implementing worker lifecycle machinery.
...
Explicit plugin invocation: `/qp-skills:alarina <request>`. Commands in the table above use this single Alárinà entrypoint; invoke a command by placing its ID first in `<request>`.
