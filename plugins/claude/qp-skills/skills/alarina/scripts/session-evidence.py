...
OWNER_PATH = re.compile(
    r"(?:^|[/\\])skills[/\\]alarina[/\\](?:owners|reference[/\\]skills)[/\\]"
    r"([a-z0-9-]+)[/\\]OWNER\.md",
    re.I,
)
TOOL_FAILURE_STATES = {"error", "failed", "failure"}
COMMAND_PATH = re.compile(
    r"(?:^|[/\\])skills[/\\]alarina[/\\]commands[/\\][a-z0-9-]+\.md",
    re.I,
)
...
def signals(o, line, names, patterns):
    ...
    for text in strings(o):
        if "alarina" in names and COMMAND_PATH.search(text):
            found.add(("alarina", "COMMAND_PATH_REFERENCE"))
        for m in SKILL_PATH.finditer(text):
            n = m.group(1)
            if n in names: found.add((n, "SKILL_PATH_REFERENCE"))
        for m in OWNER_PATH.finditer(text):
            n = m.group(1)
            if n in names: found.add((n, "OWNER_PATH_REFERENCE"))
    rank = {
        "COMMAND_PATH_REFERENCE": 1,
        "SKILL_PATH_REFERENCE": 1,
        "OWNER_PATH_REFERENCE": 1,
        "STRUCTURED_SKILL_REFERENCE": 2,
        "USER_SKILL_REFERENCE": 3,
        "EXPLICIT_INVOKE": 4,
    }
    best = {}
