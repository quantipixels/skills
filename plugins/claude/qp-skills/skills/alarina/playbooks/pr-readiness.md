...
2. For description work, return or update the reviewer brief at that stopping point. Before publication, consume [local readiness](../references/engineering/delivery/local-readiness.md) for the final candidate; reuse sufficient review and checks. Then commit and push only the scoped authorized work and read back the provider state.
