...
Decision update: Later on 29 August 2026, the owner explicitly chose package standardization without waiting for a second consumer. The resulting implementation adds the bounded `collection-filter-control.html` candidate and strengthens responsive semantic-table containment in `visual-foundation.css`. The evidence and alternatives below remain the research basis for that decision.

## Current recommendation

Add only the bounded, conditional collection filter chosen in the decision update. The current package otherwise covers the repeated standalone-artifact seams for which this repository has evidence. Prefer native HTML and small artifact-specific enhancement for the remaining evaluated patterns. Keep two focused build-time dependencies available by policy, not vendored as package assets:
...
The package currently contains four assets: `visual-foundation.css`, `theme-control.html`, `report-control.html`, and `carousel-control.html`. Its only current generated HTML consumer, [`agent-session-issue-map-2026-08-18.html`](agent-session-issue-map-2026-08-18.html), contains:
