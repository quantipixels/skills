...
| Pepeye host-adapter wiring | Claude entry preloads the existing skill; the shipped Codex profile parses as session instructions without built-in-prompt/model/permission overrides | Focused adapter tests in `.github/tests/test_package_integrity.py`; model-visible loading is not proved | STRUCTURAL |
| Pepeye supervision on Claude Code/Codex | Session/default selection, worker-role isolation, delegation, guidance, reuse, settings readback, and cancellation need authenticated runtime proof | [Host setup and read-only check](../skills/experimental/pepeye/references/hosts.md); syntax/install checks do not prove orchestration | NOT_RUN |
