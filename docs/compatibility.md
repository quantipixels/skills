...
| Pepeye supervision on Claude Code/Codex | Native main-agent setup and worker controls are documented; authenticated delegation, steering, reuse, settings readback, and shutdown are not yet runtime-proved | [Host setup and read-only check](../skills/experimental/pepeye/references/hosts.md); package/install checks do not prove orchestration | NOT_RUN |
