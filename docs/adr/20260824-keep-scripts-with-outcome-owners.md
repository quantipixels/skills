...
Superseded by [20260826-narrow-bundled-scripts-to-deterministic-seams.md](20260826-narrow-bundled-scripts-to-deterministic-seams.md) for the script-retention and runtime-state boundary. This decision remains authoritative for outcome ownership and use of native platform capabilities.
...
`amoye-ui-ux` will return one structured recommendation with one Markdown rendering. The calling workflow owns durable state and presentation. The skill will not maintain a parallel design-system directory, master file, or page-override lifecycle. Small wrappers and flags that advertise no distinct behavior will be removed. The later deterministic-seam decision narrows the provider-state, locking, retry, and candidate-proof retention described by the original version of this paragraph.
