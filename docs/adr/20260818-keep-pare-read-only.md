...
`atunwo` consumes exact-current Pare evidence in broad review and retains defects, blocking classification, verdicts, and provider operations. When another skill owns a code-review outcome, `atunwo` may fetch and pin provider content, then return a fixed candidate handoff without expanding into analysis, verdict, publication, or transferred provider authority. Amose owns reconciliation of conflicting domain vocabulary found during simplification review.
