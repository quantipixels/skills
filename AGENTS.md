...
For provider/multi-agent integrations, keep layers distinct: skills own semantic methods, workflows own progression, and assignments own the task-specific outcome, context, authority, evidence, independence, completion boundary, and worker shaping. Optional provider-native agent definitions contain only persistent deltas that cannot be expressed adequately through native spawn controls, the assignment, host defaults, or existing user configuration. The host harness owns spawn/join/scheduling/Code Mode/team mechanics. Do not introduce maintained agent-role/posture taxonomies, hardcode skill identities or workflow stages into agent definitions, or copy harness mechanics into portable skill instructions.
